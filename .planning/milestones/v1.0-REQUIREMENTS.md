...
- [x] **PROG-01**: Skill defaults to solving the immediate request without interrogation or phase gates
- [x] **PROG-02**: Skill escalates to structured questioning after 2-3 unresolved attempts, not upfront
- [x] **PROG-03**: Escalation surfaces targeted questions (only what's blocking the solution), not a full pre-flight checklist
- [x] **PROG-04**: Deep workflow mode (full phase breakdown + gates) only activates when user explicitly requests it

### Interrogation

- [x] **INTR-01**: When escalating, skill extracts the business question the report needs to answer
- [x] **INTR-02**: When escalating, skill gathers data model state (tables, relationships, calculated columns)
- [x] **INTR-03**: When escalating, skill audits existing measures to prevent duplication or conflicts
